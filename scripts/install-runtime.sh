...
ngen_reaper() {  # $1 = winetricks pid to track
...
for verb in "$@"; do
  ...
  # Launch winetricks FIRST and capture its pid, so the reaper tracks the real
  # process (not a racy pgrep that can miss it before it starts / after it ends).
  /usr/bin/arch -x86_64 "$WINETRICKS" -q "$verb" & WT=$!
  ngen_reaper "$WT" & WD=$!
  rc=0; wait "$WT" || rc=$?
  kill "$WD" 2>/dev/null || true
  pkill -9 -f 'mscorsvw.exe' 2>/dev/null; pkill -9 -f 'ngen.exe' 2>/dev/null
  [[ "$rc" -eq 0 ]] || warn "winetricks '$verb' returned non-zero (often OK if files landed)."
done
